...
function buildEvent(attributes) {
  if (!attributes || _.isEmpty(attributes)) {
    ...
    .concat([
      generateProdId(attributes.prodId),
      'BEGIN:VEVENT',
  ...
  function generateProdId(prodId) {
    if(prodId) {
      return 'PRODID:' + prodId;
    }
    return 'PRODID:-//Adam Gibbons//agibbons.com//ICS: iCalendar Generator';
  }
  ...
  function formatDTEND(startString, endString, tz, tzEnd) {
    ...
    if (endString && isDateTime(startString) && moment.parseZone(startString).utcOffset() === 0 && !isUTC(string)) {
      // Form #1: DATE WITH LOCAL TIME
